  ...
  SlashCommand: {
    ...
    */
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      if (!user)
        return interaction.send({ content: `You cannot logout if your not logged in.` });

      client.dbo.collection("users").updateOne({ "user.discord.id": interaction.member.user.id }, {
        $unset: {
          "user.discord": ""
        },
        $set: {
