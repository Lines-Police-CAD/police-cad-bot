...
class LinesPoliceCadBot extends Client {
  constructor(options, config) {
    ...
    this.ws.on("INTERACTION_CREATE", async (interaction) => {
      if (interaction.type != 3) {
        ...
        interaction.editOriginal = async (message) => {
          const rest = new REST({ version: "10" }).setToken(
            client.config.Token,
          );
          // webhookId for interactions = application id
        ...
        try {
          await cmd.SlashCommand.run(this, interaction, args, { GuildDB });
        } catch (err) {
  ...
  async connectMongo(mongoURI, dbo) {
    ...
    try {
      ...
      let db = (mongoURI.includes("@")
        ...
        : mongoURI;
      this.error(
        `Failed to connect to mongodb (mongodb://${db}/${dbo}): attempt ${databaselogs.attempts} - ${err}`,
      );
      failed = true;
    }
  ...
  async checkRoleStatus(rolesCache, allowedRoles) {
    ...
    for (let i = 0; i < allowedRoles.length; i++) {
      if (rolesCache.includes(allowedRoles[i]))
        return true; // they have at least one matching role
      else continue;
    }
  ...
  async GetGuild(GuildId) {
    let guild = await this.dbo
      .collection("prefixes")
      .findOne({ "server.serverID": GuildId })
      .then((guild) => guild);
    ...
    let guildData = {
      ...
      customChannelStatus:
        guild.server.allowedChannels.length > 0 ? true : false,
  ...
  async verifyUseCommand(serverID, rolesCache) {
    ...
    for (let i = 0; i < guildDB.allowedRoles.length; i++) {
      let role = guild.roles.cache.find(
        (role) => role.id == guildDB.allowedRoles[i],
      );
      if (!role)
        update = true; // this role no longer exists, we need to update, dont add to list
